...
suite "client integration":
  ...
  test "execute DDL":
    var c = open("127.0.0.1", TestPort)
    defer: c.close()
    c.execute("DROP TABLE IF EXISTS test_nim")
    c.execute("""
    ...
    """)
    c.execute("DROP TABLE test_nim")

  test "query select 1":
    var c = open("127.0.0.1", TestPort)
    defer: c.close()
    let res = c.query("SELECT 1 AS n")
  ...
  test "query multiple rows":
    var c = open("127.0.0.1", TestPort)
    defer: c.close()
    let res = c.query("SELECT number FROM system.numbers LIMIT 10")
  ...
  test "insert and select round-trip":
    var c = open("127.0.0.1", TestPort)
    defer: c.close()
    c.execute("DROP TABLE IF EXISTS test_rt")
    c.execute("""
    ...
    c.insert("INSERT INTO test_rt (id, name, val) VALUES", blk)
    let res = c.query("SELECT id, name, val FROM test_rt ORDER BY id")
  ...
  test "integer types round-trip":
    var c = open("127.0.0.1", TestPort)
    defer: c.close()
    let res = c.query("SELECT toUInt8(255), toInt32(-42), toUInt64(18446744073709551615), toFloat32(3.14)")
    check res.num_rows == 1
